function varargout = differentialevolutionparallel(varargin)
...
getparametername(paramDefCell, parameterDimVector,0,0);
%displaybestmember(paramDefCell, parameterDimVector, 0, 0, ...
 %               0, 0, 0, 1, 0, optimInfo, 0, 0);

% get parameter bounds
parameterBounds = cell2mat(paramDefCell(:,2));
parGridVector   = cell2mat(paramDefCell(:,3));
D               = size(parameterBounds, 1);
subD            = DEParams.subFoM;
...
if useInitParams && (isempty(optimResult) || isempty(optimResult.currentPopulation))
    ...
    if size(paramDefCell, 2) == 4
        ...
        while parNr <= D
            if ~isempty(paramDefCell{parNr, 4})
                ...
                parameterName = getparametername(paramDefCell, parameterDimVector, parNr, 2);
            ...
            for d = 1:parameterDimVector(parNr)
                ...
                if isnan(initialMem(index))
                    parameterName = getparametername(paramDefCell, parameterDimVector,index, 1);
                    disp(textwrap2(sprintf(...
    ...
    elseif isempty(objFctParams)
        ...
        elseif strcmp(paramDefCell{1,1}, '_1')
            ...
            for parNr = 1:D
                parameterName = getparametername(paramDefCell, parameterDimVector,parNr, 1);
        ...
        for parNr = 1:D
            if (~isnan(initialMem(parNr))) && (paramDefCell{parNr,3} > 0) && ...
                ...
                disp(textwrap2(sprintf(...
                    ...
                    'grid value: %g).'], getparametername(paramDefCell, parameterDimVector,parNr, 1), initialMem(parNr), ...
...
if isempty(optimResult)
    ...
    subval          = nan(NP, subD);    % non optimised cost vector returnhere
    ...
    allsubval       = [];               % vector with all computed sub cost values
    ...
    subval          = optimResult.currentSubCostValues;
    ...
    allsubval       = optimResult.allSubEvaluationValues;
...
while ...
    ...
    if initialization || reinitialization
        ...
        parfor memberNr = 1:NP          
            % evaluate member
            [val(memberNr), subval(memberNr,:), pop(memberNr,:), ~, ~, ~, ~] = computeevaluationvalue(...
                pop(memberNr,:), nfeval, timeOver, objFctParams, paramDefCell, parameterDimVector, ...
                objFctSettings, objFctHandle, saveHistory, allval, allsubval, allmem, iterationNr, memberNr, ...
                NP, slaveFileDir, maxMasterEvals, validChkHandle, XVmin, XVmax); %#ok   
        ...
        if initialization
            ...
            if saveHistory
                saveoptimizationstate(...
                    ...
                    bestval, bestmem, 0, 0, pop, val, subval, startTime, state, DEParams, allval, allsubval, allmem, ...
        ...
        if displayResults && saveHistory
            %displayoptimizationhistory(paramDefCell, allmem, allval,allsubval);
            %drawnow;
            GUI(paramDefCell(:,1), allmem, allval,allsubval);
        end
    ...
    parfor memberNr = 1:NP
        ...
        [tempval, subtempval, popnew(memberNr,:), ~, ~, ~, ~] = computeevaluationvalue(...
            popnew(memberNr,:), nfeval, 0, objFctParams, paramDefCell, parameterDimVector, ...
            objFctSettings, objFctHandle, saveHistory, allval, allsubval, allmem, iterationNr, memberNr, NP, ...
...
if saveHistory
    saveoptimizationstate(...
        paramDefCellInput, paramDefCell, parameterDimVector, optimInfo, bestval, bestmem, ...
        bestvalhist, bestmemhist, valstddevhist, paramstddevhist, pop, val, subval, startTime, state, ...
        DEParams, allval, allsubval, allmem, objFctName, objFctHandle, objFctSettings, objFctParams, ...
        emailParams, 1, 1);
end
disp(repmat('*', 1, textWidth));
...
for parNr = 1:D
    if XVmin(parNr) > XVmax(parNr)
        disp(textwrap2(sprintf(...
            ...
            XVmin(parNr), getparametername(paramDefCell, parameterDimVector,parNr, 1), XVmax(parNr))));
        errorFound = true;
    end
    if parGridVector(parNr) < 0
        disp(textwrap2(sprintf(...
            'Error: Negative quantization values are not allowed (parameter %s).', ...
            getparametername(paramDefCell, parameterDimVector,parNr, 1))));
        errorFound = true;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function str = getparametername(paramDefCell,parameterDimVector,parNr,nameMode)
...
function varargout = saveoptimizationstate(...
    ...
    bestmemhist, valstddevhist, paramstddevhist, pop, val, subval, startTime, state, DEParams, allval, allsubval, ...
...
if forceSaveFile || mbtime - nextSaveTime >= 0
    ...
    optimResult.currentSubCostValues     = subval;
    optimResult.allEvaluationValues      = allval;
    optimResult.allSubEvaluationValues   = allsubval;
    ...
    if ~isempty(bestmem)
        ...
        if strcmp(paramDefCell{1,1}, '_1')
            ...
            while parNr <= D
                index = parNr : (parNr + parameterDimVector(parNr) - 1);
                optimResult.bestFctParams.(getparametername(paramDefCell, parameterDimVector,parNr, 2)) = bestmemQuantized(index);
                parNr = parNr + parameterDimVector(parNr);
            end
        end
...
if nargin == 1
    ...
    maxNameLength = 0;
    for parNr = 1:D
        maxNameLength = max(maxNameLength, length(getparametername(paramDefCell, parameterDimVector,parNr, 1)));
    end
    return
end
...
if ~any(isnan(lastbestmem)) && any(size(bestmem) ~= size(lastbestmem))
    display('Internal error: bestmem and lastbestmem are of different size!');
    return;
end
...
if ~strcmp(paramDefCell{1,1}, '_1')
    ...
    while parNr <= length(paramVec)
        index = parNr:parNr+parameterDimVector(parNr)-1;
        objFctParams.(getparametername(paramDefCell, parameterDimVector,parNr,2)) = paramVec(index)';
        parNr = parNr + parameterDimVector(parNr);
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [testval, subtestval, testmem, nfeval, allval, allsubval, allmem] = computeevaluationvalue(...
    testmem, nfeval, timeOver, objFctParams, paramDefCell, parameterDimVector, objFctSettings, ...
    objFctHandle, saveHistory, allval, allsubval, allmem, iterationNr, memberNr, NP, slaveFileDir, ...
...
if ~isempty(allmemIndex)
    ...
    subtestval   = allsubval(allmemIndex);
    ...
    if ~isempty(objFctHandle)
        ...
        if isempty(testval)  && ~timeOver
            ...
            if iscell(testval)
                subtestval = cell2mat(testval(2:end));
                testval = testval{1};
            end
        ...
        %if isempty(optParVector)
            optParVector = XVmin + rand(1, length(XVmin)) .* (XVmax - XVmin); %% Parallel warning.
        %end
    ...
    if ~isempty(testval)
        ...
        if saveHistory
            allmem = [allmem,testmem'];
            allval = [allval,testval];
            allsubval = [allsubval, subtestval];
        end
    else
        testval = NaN;
        subtestval = NaN;
