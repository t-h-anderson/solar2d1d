...
sim.input = struct(...
    ...
    'icm3isG', -inf ...   % Generation Rate (value, function, file, or -inf for results from RCWA)
);

% Design the junction
sim.material = struct(...
    'nsec', 9, ...         % Number of independent section in study
    'elecsec', [0,0,0,1,1,1,0,0,0], ... % Flags for which sections to include in the electrical simulation
    'periodicsec', [0,0,0,0,0,0,0,1,0], ... % ??? grating? Flags for which sections to include in the electrical simulation
    'nmSec', [100,110,100,10,25,20,20,1,100], ... % Lengths of the sections
    'icm3ND', [0,0,0,1e17,5e17,-1e16,0,0,0], ... % Doping of the sections
    ...
    'VEg0', [0,0,0,3.3,2.4,1.13,0,0,0], ... % Baseline bandgap
    ...
    ...%'VChi',0,...  % Electron Affinity
    'epsdc', 11.68, ... % DC relative permittivity of bulk silicon
    ... % Recombination Parameters
    'icm3isalpha', 1e-10, ... % Radiative recombination constant 1/(cm^3 s), related to beta via beta = alpha/ni^2
    'istausrhn', 0e-12, ... % SRH recombination constant 1/s
    'istausrhp', 0e-9, ... % SRH recombination constant 1/s
    ...
    ... % Optical Properties
    'eps', [1, 1,1,1,1,15+2i,1,1, 1] ... %[1+1e-9i, 4, 11.559+0.204i, 11.559+0.204i, -10.4356+0.8294i] ... % Optical Permittivit
   ...% 'VEg1',strcat('0&0&0&0&0&@(x)',num2str(0.390),'*(0.5*(1+sin(',num2str(2),'*2*pi*x - ',num2str(.75),'*2*pi))).^',num2str(6),'&0&0&0'),...%%%;
   ...% 'VChi1', strcat('0&0&0&0&0&@(x)',num2str(4.46),'-',num2str(0.16),'*(0.5*(1+sin(',num2str(2),'*2*pi*x - ',num2str(0.75),'*2*pi))).^',num2str(6),'&0&0&0'),...%%;
   ...% 'istausrhn1',strcat('0&0&0&0&0&@(x)',num2str(1.9537e-09),'-',num2str(1.8370e-09),'*(0.5*(1+sin(',num2str(2),'*2*pi*x - ',num2str(0.75),'*2*pi))).^',num2str(6),'&0&0&0'),...%;
    ...%'istausrhp1',strcat('0&0&0&0&0&@(x)',num2str(1.9537e-09),'-',num2str(1.8370e-09),'*(0.5*(1+sin(',num2str(2),'*2*pi*x - ',num2str(0.75),'*2*pi))).^',num2str(6),'&0&0&0')... 
);
