function [sim] = DesignJunctionOpticTest()
...
sim.material = struct(...
    'nsec', 3, ...         % Number of independent section in study
    'elecsec', [0, 0, 0], ... % Flags for which sections to include in the electrical simulation
    'periodicsec', [0, 1, 0], ... % Flags for which sections to include in the electrical simulation
    'nmSec', [1500, 50, 50], ... % Lengths of the sections
    'icm3ND', [0, 0, 0], ... % Doping of the sections
    ...
    'material', 'Air & Air & user' , ... % Loads a material from the database, 0 for user input
    'VEg0', 0, ... % Baseline bandgap
    ...
    ... % Optical Properties
    'eps', [1, 1, -15+4i] ... % Optical Permittivity
    );
...
sim.optical.periodic = struct(...
    ...
    'zeta', '@(z) 0.5*(1-z)', ... %'@(z) (1/pi) * acos(z/80)', ...         % Graing Duty Cycle
    ...
    'eps2', [-15+4i] ...                                   % Grating metal permittivity
    );
