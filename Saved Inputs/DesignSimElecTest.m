function sim = DesignSimElecTest(sim)
...
sim.optical.setup = struct(...
    ...
    'nmlambda0', 350, ...   % Minimum wavelength in nm
    'nmlambda1', 1100, ...  % Maximum wavelength in nm
    'nlambda', 1 + floor((1100-350)/5), ...     % Number of wavelengths
    'Nx', 100, ...       % Number sample points in x-direction
    'nslices', [1, 1, 20, 200, 20, 1, 1], ... % Number of slices in each sec (DesignJunction.m), uses 1 if not set.
    ...
    'maxNt', 10, ...        % Largest value for Nt (tom:40)
...
sim.electrical.setup = struct(...
    ...
    'nx_sec', [10,100,10], ... % Number of mesh points in each section
  ...
  ... % Tolerance
    ...
    'trydamping', 1, ... % >=1 Number of times damping is increases to aid convergence
    'dampingincrease', 10, ... Amount to increase damping by each attempt
    'mesh_increase', 10, ... mesh multiplier on fail
    'refine_on_negative', 1, ...
    'refine_on_Jnoise', 1, ...
    'maxJnoiserefinefrac', 0.2, ...
    'refine_always', 0, ...
    'maxloop', 100, ... % Maximum number of iteration for convergence
    'maxpdeg', 8, ... %
    'maxtime', 60*60, ... % Maximum time allowed for the initial HDG convergence
    'mindV', 1e-6, ...
    'reltol', 1e-4 ,... % Required relative tolerance
    'abstol', 1e-6, ... % Required absolute tolerance   
    ...
    'PVtol', 1e-6 ... % Tolerance for finding Pmax
   );
